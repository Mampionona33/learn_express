...
const updateUser = (req, res) => {
  const id = req.params.id * 1;
  const selectedUser = tours.filter((el) => el._id === id);
  const updatedUsers = tours.filter((el) => el._id !== id);
  ...
  // Validation
  if (id > tours.length - 1 || selectedUser.length <= 0) {
    console.log(id, tours.length);
    return res.status(404).json({ message: 'Invalid id' });
  }
...
const userByIdBasedUrl = '/api/v1/users/:id';
app.route(userBasedUrl).get(getUsers).post(createUser);
app.route(userByIdBasedUrl).get(getUser).patch(updateUser).delete(deleteUser);
